...
namespace Warehouse.Services.Services
{
    public class SlideService : ISlideService
    {
        ...
        {
            return _slideDal.GetList().OrderBy(x => x.Order).ToList();
        }
        ...
        public List<Slide> GetByStatus(bool status)
        {
            return _slideDal.GetList(x => x.Status == status).OrderBy(x => x.Order).ToList();
        }
    }
}
